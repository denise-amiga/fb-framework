...
scope
  ...
  /'
    Note that, while you can indeed add both types of elements
    to priority queues (by reference and by pointer), and they
    will get collected when you 'clear()' the queue or it goes
    out of scope, when you're dequeuing you will have a problem,
    because you can't discriminate whether or not you should dispose
    of the dequeued element. Returning an auto_ptr is also not a
    possibility (because this may not be what you intend), so this
    example will indeed cause a leak.
...
/'
  This is the correct usage: we will pass all elements by pointer to the
  queue, and delete them after we retrieved and used them.
'/
